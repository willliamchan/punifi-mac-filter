...
> **Design commitments, not an implemented security claim.** This repository is documentation-only. These requirements must be verified before release.

## Strictly one-way

pfSense is authoritative. The integration will read DHCP policies and static MAC mappings and write only the MAC-filter fields on explicitly mapped UniFi WLANs.

It will not write to pfSense, synchronize UniFi changes back to the firewall, edit SSIDs/VLANs/Wi-Fi passwords, manage wired switch ports, or enroll new targets automatically. A sync button uses the same guarded path as automatic sync.

## Credentials and connection security

- Use certificate-verified HTTPS. No HTTP fallback, redirect following or TLS-verification bypass is planned.
...
- Credentials will use Home Assistant's normal config-entry storage, not a separate vault. Masked UI fields are not at-rest encryption. Someone with sufficient access to HA's configuration or backups may recover them.
...
The planned first-run default is observation-only. Writes require explicit global and per-mapping enablement, fresh valid source/target observations, a confirmed mapping and supported policy semantics.
...
- No installable build, supported-version matrix or end-to-end HA/HACS validation is available yet.
...
- Public documentation describes the planned behavior. Release notes and tested-version evidence will identify what has actually been delivered.
